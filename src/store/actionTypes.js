export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'
export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS'

export const INSERT_USER_START = 'INSERT_USER_START'
export const INSERT_USER_SUCCESS = 'INSERT_USER_SUCCESS'
export const INSERT_USER_FAILED = 'INSERT_USER_FAILED'

export const SEARCH_FRIEND_START = 'SEARCH_FRIEND_START'
export const SEARCH_FRIEND_SUCCESS = 'SEARCH_FRIEND_SUCCESS'
export const SEARCH_FRIEND_FAILED = 'SEARCH_FRIEND_FAILED'

export const ADD_FRIEND_START = 'ADD_FRIEND_START'
export const ADD_FRIEND_SUCCESS = 'ADD_FRIEND_SUCCESS'
export const ADD_FRIEND_FAILED = 'ADD_FRIEND_FAILED'

export const GET_USER_START = 'GET_USER_START'
export const GET_USER_SUCCESS = 'GET_USER_SUCCESS'
export const GET_USER_FAILED = 'GET_USER_FAILED'

export const GET_DEBIT_START = 'GET_DEBIT_START'
export const GET_DEBIT_SUCCESS = 'GET_DEBIT_SUCCESS'
export const GET_DEBIT_FAILED = 'GET_DEBIT_FAILED'

export const GET_CREDIT_START = 'GET_CREDIT_START'
export const GET_CREDIT_SUCCESS = 'GET_CREDIT_SUCCESS'
export const GET_CREDIT_FAILED = 'GET_CREDIT_FAILED'

export const GET_GROUPS_START = 'GET_GROUPS_START'
export const GET_GROUPS_SUCCESS = 'GET_GROUPS_SUCCESS'
export const GET_GROUPS_FAILED = 'GET_GROUPS_FAILED'

export const GET_MEMBERS_START = 'GET_MEMBERS_START'
export const GET_MEMBERS_SUCCESS = 'GET_MEMBERS_SUCCESS'
export const GET_MEMBERS_FAILED = 'GET_MEMBERS_FAILED'

export const GET_GROUP_INC_START = 'GET_GROUP_INC_START'
export const GET_GROUP_INC_SUCCESS = 'GET_GROUP_INC_SUCCESS'
export const GET_GROUP_INC_FAILED = 'GET_GROUP_INC_FAILED'
